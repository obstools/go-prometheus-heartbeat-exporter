...
func TestNewLogger(t *testing.T) {
	t.Run("when log to stdout, server activity enabled", func(t *testing.T) {
		isLogToStdoutEnabled, logActivity := true, true
		logger := &eventLogger{
		...
		assert.EqualValues(t, logger, newLogger(isLogToStdoutEnabled, logActivity))
	})

	t.Run("when log to stdout, server activity disabled", func(t *testing.T) {
		isLogToStdoutEnabled, logActivity := false, false
		logger := &eventLogger{
			flag:   logFlag,
			stdout: os.Stdout,
			stderr: os.Stderr,
